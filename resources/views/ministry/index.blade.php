            ...
            <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 max-w-7xl" data-aos="fade-up" data-aos-duration="1500">
                ...
                <div class="max-w-7xl overflow-hidden shadow-md hover:shadow-xl transition-all duration-300 relative">
                    <div class="mcg p-6">
                        ...
                        <p class="text-sm sm:text-base lg:text-lg text-gray-600-300 mb-6 line-clamp-2">{!! nl2br(e($ministry->description)) !!}</p>
                        ...
                        <div class="flex gap-2">
                            <button onclick="openModal('modal-{{ $ministry->id }}')"
                                class="more text-center text-sm lg:text-base flex-1 rounded-lg transition-colors font-medium">
                                See More
                            </button>
                            @if($ministry->status === 'open' && $ministry->users->count() < $ministry->total_slots)
                                <form action="{{ route('ministry.join', $ministry) }}" method="POST" class="flex-1">
                                    @csrf
                                    <button type="submit"
                                        class="join text-center text-sm lg:text-base w-full text-white px-4 py-2 rounded-lg transition-colors font-medium">
                                        Join Now
                                    </button>
                                </form>
                                @else
                                <button disabled
                                    class="close text-center text-sm lg:text-base px-4 py-2 flex-1 rounded-lg cursor-not-allowed font-medium">
                                    {{ $ministry->status === 'closed' ? 'Closed' : 'Full' }}
                                </button>
                                @endif
                                @if(auth()->user() && auth('web')->user()->isAdmin())
                                <a href="{{ route('ministries.edit', $ministry->id) }}"
                                    class="text-sm lg:text-base text-center flex-1 bg-yellow-400 text-black rounded-lg">
                                    Edit
                                </a>
                                <a href="{{ route('ministries.users', $ministry->id) }}"
                                    class="text-sm lg:text-base text-center flex-1 bg-red-400 text-white px-4 py-2 rounded-lg">
                                    Manage Users
                                </a>
                                @endif
                        </div>
                        ...
                        <div class="min-h-screen px-4 text-center">
                            <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full">
                                <div class="relative p-6">
                                    <button onclick="closeModal('modal-{{ $ministry->id }}')" class="bx absolute top-4 right-4 z-20 bg-white rounded-full p-1 shadow-lg hover:bg-gray-100">
                                        <svg class="h-6 w-6 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                                        </svg>
                                    </button>
                                    <h3 class="text-lg font-medium">{{ $ministry->name }}</h3>
                                    <p class="text-sm">{!! nl2br(e($ministry->description)) !!}</p>
                                </div>
                            </div>
                        </div>
                    </div>
